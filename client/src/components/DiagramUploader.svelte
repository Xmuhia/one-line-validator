<script>
    ...
    // Import Dropzone as a component rather than a module
    let DropzoneComponent;
    
    onMount(async () => {
      if (browser) {
        try {
          // Import the specific component directly
          const module = await import('svelte-file-dropzone');
          DropzoneComponent = module.default || module.Dropzone;
          
          if (!DropzoneComponent) {
            console.error('Failed to load Dropzone component', module);
            error = 'Failed to load file upload component. Please refresh the page.';
          }
        } catch (err) {
          console.error('Error loading Dropzone:', err);
          error = 'Failed to load file upload component. Please refresh the page.';
        }
      }
    });
...
<div class="diagram-uploader">
  ...
  <div class="form-group">
    {#if browser && DropzoneComponent}
      <svelte:component this={DropzoneComponent} on:drop={handleFilesSelect} accept=".png,.jpg,.jpeg,.pdf">
